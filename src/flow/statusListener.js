...
// Menyimpan ID pesanan yang sedang diproses agar tidak dobel eksekusi
...
function startStatusListener() {
  if (isListenerStarted) {
    logger.info('⚠️ Status Listener sudah berjalan, skip inisialisasi ganda.');
    return;
  }
      ...
      async (payload) => {
        ...
        try {
          ...
          if (newOrder.order_status === 'confirmed' && newOrder.payment_status !== 'paid') {
            if (processingOrders.has(newOrder.id)) return;
            processingOrders.add(newOrder.id);
            try {
              ...
              processingOrders.delete(newOrder.id);
            }
          }
    ...
    try {
      ...
      if (confirmedOrders && confirmedOrders.length > 0) {
        for (const newOrder of confirmedOrders) {
          if (processingOrders.has(newOrder.id)) continue;
          processingOrders.add(newOrder.id);
          try {
